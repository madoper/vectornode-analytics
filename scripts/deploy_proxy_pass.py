...
# Chart POST with slice_id (URL-encoded) - should now return chart data directly
fd = urllib.parse.quote('{"slice_id":5}')
url = 'https://bi.vectornode.ru/api/v1/chart/data?form_data=' + fd + '&dashboard_id=2'

_, o3, _ = ssh.exec_command(
    'curl -s -k -w "\nHTTP_%{http_code}" -X POST "' + url + '" '
    '-H "Authorization: Bearer ' + token + '"'
)
print('\nChart POST (encoded, no redirect):')
print(o3.read().decode(errors='replace')[:500])

# Native filter POST (no slice_id)
