...
print('Result:', out[:500])

# Check error logs
_, so2, _ = ssh.exec_command('docker logs podft-superset --since 1m 2>&1 | grep -E "QC_LOAD|FILTER_|Traceback|Error" | tail -10')
print('\nDebug logs:', so2.read().decode(errors='replace') or 'NONE')
