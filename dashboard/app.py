...
if page == 0:
    ...
    with engine.connect() as conn:
        hs = pd.read_sql(text(Q_HYPOTHESIS_SUMMARY), conn.connection)
...
# ======================== PAGE 1: COMPANY ========================
    ...
    with engine.connect() as conn:
        tl = pd.read_sql(text(Q_COMPANY_TIMELINE), conn.connection, params={"cid": cid})
        an = pd.read_sql(text(Q_COMPANY_ANOMALIES), conn.connection, params={"cid": cid})
        fl = pd.read_sql(text(Q_COMPANY_FLAGS), conn.connection, params={"cid": cid})
    if tl.empty:
...
# ======================== PAGE 2: HYPOTHESES ========================
    ...
    with engine.connect() as conn:
        sc = pd.read_sql(text(Q_ANOMALY_SCATTER), conn.connection, params={"h": h, "y": None if y == "Все" else int(y)})
...
# ======================== PAGE 3: GROUPS ========================
    ...
    with engine.connect() as conn:
        grp = pd.read_sql(text(Q_GROUPS), conn.connection, params={"gtype": gtype})
    if grp.empty:
...
# ======================== PAGE 4: INDUSTRY ========================
    ...
    with engine.connect() as conn:
        ind_margin = pd.read_sql(text(Q_INDUSTRY_MARGIN), conn.connection)
        ind_tax = pd.read_sql(text(Q_INDUSTRY_TAX), conn.connection)
